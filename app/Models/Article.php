...
class Aritcle extends Base
{
    ...
    {
        return $this->only('id','title','description','keywords','markdown');
    }
    /**
     * 过滤描述中的换行。
     *
    ...
    public function storeData($data, $flash = true)
    {
        ...
        // 给文章的插图添加水印;并取第一张图片
        $firstImg = $this->getCover($data['markdown']);
        if(empty($data['cover'])){
            $data['cover'] = $firstImg;
        }
     ...
     */
     ...
     public function searchArticleId($wd)
     {
